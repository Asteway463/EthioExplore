...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "EthioExplore — Discover the Beauty of Ethiopia" },
      {
        name: "description",
        content:
          "Plan trips to Lalibela, the Simien Mountains, Danakil and more. Curated Ethiopian destinations, itineraries and budgets in USD or ETB.",
      },
      { property: "og:title", content: "EthioExplore — Discover the Beauty of Ethiopia" },
      {
        property: "og:description",
        content: "Curated Ethiopian destinations, itineraries and budget planning in USD or ETB.",
      },
    ],
  }),
  component: Index,
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-5 py-28 md:px-8 md:py-36">
          ...
          <div className="fade-in mt-6 flex flex-wrap gap-3">
            <Link
              to="/planner"
              className="flex items-center gap-2 rounded-full bg-gold px-5 py-3 text-sm font-semibold text-gold-foreground transition hover:brightness-95"
            >
      ...
      <section className="relative z-10 mx-auto -mt-10 grid max-w-7xl grid-cols-2 gap-4 px-5 md:-mt-14 md:grid-cols-4 md:px-8">
        {STATS.map((s) => (
          <div key={s.l} className="card-lift rounded-2xl bg-card p-5 text-center shadow-lift">
        ...
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {TESTIMONIALS.map((t) => (
              <div
                key={t.name}
                className="card-lift rounded-2xl border border-white/10 bg-white/5 p-6"
              >
                <RatingStars rating={t.rating} />
                <p className="mt-4 text-sm leading-relaxed text-ink-foreground/80">“{t.text}”</p>
                <div className="mt-5 flex items-center gap-3">
                  <div className="surface-brand flex size-9 items-center justify-center rounded-full text-xs font-bold">
                    {t.name[0]}
                  </div>
                  <div>
      ...
      <section className="mx-auto max-w-5xl px-5 py-20 text-center md:px-8">
        <div className="surface-cta relative overflow-hidden rounded-3xl p-10 text-primary-foreground md:p-14">
          <RouteMotif />
          <div className="relative">
            ...
            <Link
              to="/planner"
              className="inline-block rounded-full bg-background px-6 py-3 text-sm font-semibold text-primary transition hover:bg-mist"
            >
              Start Planning
            </Link>
          </div>
